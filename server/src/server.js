...
// ---------------- Timesheets completeness watchdog (weekly/monthly) ----------------
// Rules:
...
async function computeTimesheetTotals({ datum_von, datum_bis, unit }){
  const items = await getAggregatedItems({ type: 'zeiten', unit, datum_von, datum_bis })
  const toNumber = (n)=>{ if(n==null)return 0; if(typeof n==='number')return n; if(typeof n==='string'){const s=n.replace(/\./g,'').replace(',', '.'); const v=Number(s); return isNaN(v)?0:v} return Number(n||0) }
  const byEmp = new Map()
  const empSet = new Set()
  for (const r of items){
    const emp = (r?.MITARBEITER ?? r?.mitarbeiter ?? '—').toString(); empSet.add(emp)
    const val = toNumber(r?.stunden_gel ?? r?.STD_GELEISTET ?? r?.STUNDEN ?? r?.stunden ?? r?.ZEIT ?? r?.zeit ?? 0)
    if (!val) continue
    byEmp.set(emp, (byEmp.get(emp)||0) + val)
  }
  return { byEmp, empSet }
}
...
async function runTimesheetsWatchdog({ mode = 'weekly', unit = 'ALL', recipients = [], hoursPerDay = 8 }){
  ...
  if (mode === 'monthly'){
    ...
    const day = now.getUTCDay() || 7
    const fri = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() - day + 5, 23,59,59))
    const mon = new Date(Date.UTC(fri.getUTCFullYear(), fri.getUTCMonth(), fri.getUTCDate() - 4, 0,0,0))
    range = { datum_von: toIsoStringUTC(mon), datum_bis: toIsoStringUTC(fri) }
  }
  ...
  const subject = `Erfassungs-Kontrolle (${mode}) · ${unit||'ALL'} · ${range.datum_von.slice(0,10)}–${range.datum_bis.slice(0,10)}`
  const rec = (Array.isArray(recipients) ? recipients : [recipients]).filter(Boolean)
  if (rec.length > 0){ await sendSmtpMail({ to: rec, subject, html, text: html.replace(/<[^>]+>/g,' '), attachments }) }
  return { range, rows, offenders }
}

// JSON
app.get('/api/watchdogs/timesheets/report', async (req, res) => {
  try{
    const unit = req.query.unit || 'ALL'
...
// Preview page
app.get('/api/watchdogs/timesheets/preview-page', async (req, res) => {
  function esc(s){ return String(s||'').replace(/[&<>]/g, c=> ({'&':'&amp;','<':'&lt;','>':'&gt;'}[c])) }
  try{
    const unit = req.query.unit || 'ALL'
    const mode = (req.query.mode === 'monthly') ? 'monthly' : 'weekly'
...
// Build a simple PDF for reports (Umsatzliste or Stunden) using PDFKit
async function generateReportPdf({ type = 'zeiten', unit = 'ALL', datum_von, datum_bis }){
  // Fetch raw aggregated rows (handles unit=ALL internally)
  ...
  const hoursFmt = (n)=> new Intl.NumberFormat('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(n||0))
  const moneyFmt = (n)=> new Intl.NumberFormat('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(n||0))

  let rows = []
  if (type === 'umsatzliste'){
    // Expect fields: KUNDE/PROJEKT/UMSATZ (case-insensitive)
    const map = new Map()
    for (const r of items){
      const kunde = r?.KUNDE ?? r?.kunde ?? r?.KUNDENNAME ?? 'Unbekannt'
      const umsatz = toNum(r?.UMSATZ ?? r?.umsatz ?? r?.UMSATZ_TATSAECHLICH ?? r?.umsatz_tatsaechlich)
      const cur = map.get(kunde) || { kunde, umsatz: 0 }
      cur.umsatz += umsatz
      map.set(kunde, cur)
    }
    rows = Array.from(map.values()).sort((a,b)=> Number(b.umsatz||0) - Number(a.umsatz||0))
  } else {
    // Stunden: sum hours per employee
    const map = new Map()
    for (const r of items){
      const emp = (r?.MITARBEITER ?? r?.mitarbeiter ?? '—').toString()
      const h = toNum(r?.STUNDEN ?? r?.stunden ?? r?.STD_GELEISTET ?? r?.stunden_gel)
      if (!map.has(emp)) map.set(emp, { mitarbeiter: emp, stunden: 0 })
      map.get(emp).stunden += h
    }
    rows = Array.from(map.values()).sort((a,b)=> Number(b.stunden||0) - Number(a.stunden||0))
  }
  ...
  // Header
  doc.fontSize(16).text('Realcore Controlling Report', { align: 'left' })
  doc.moveDown(0.25)
  const title = type === 'umsatzliste' ? 'Umsatzliste' : 'Stundenübersicht'
  doc.fontSize(12).fillColor('#444').text(`${title} · Unit: ${unit||'ALL'} · Zeitraum: ${String(datum_von||'').slice(0,10)} – ${String(datum_bis||'').slice(0,10)}`)
  doc.moveDown(0.5)
  doc.fillColor('#000')

  // Table header
  const startX = 40, startY = doc.y + 10
  let x = startX, y = startY
  doc.fontSize(11).fillColor('#000')
  if (type === 'umsatzliste'){
    doc.text('Kunde', x, y, { width: 240 }); x += 260
    doc.text('Umsatz', x, y, { width: 120, align: 'right' })
    y += 18
    doc.moveTo(startX, y).lineTo(550, y).strokeColor('#ddd').stroke(); y += 6
    // rows
    for (const r of rows){
      x = startX
      doc.text(r.kunde, x, y, { width: 240 }); x += 260
      doc.text(moneyFmt(r.umsatz), x, y, { width: 120, align: 'right' })
      y += 16
      if (y > 780) { doc.addPage(); y = 50 }
    }
  } else {
    doc.text('Mitarbeiter', x, y, { width: 240 }); x += 260
    doc.text('Stunden', x, y, { width: 120, align: 'right' })
    y += 18
    doc.moveTo(startX, y).lineTo(550, y).strokeColor('#ddd').stroke(); y += 6
    for (const r of rows){
      x = startX
      doc.text(r.mitarbeiter, x, y, { width: 240 }); x += 260
      doc.text(hoursFmt(r.stunden), x, y, { width: 120, align: 'right' })
      y += 16
      if (y > 780) { doc.addPage(); y = 50 }
    }
  }
...
async function runSchedule(s) {
  ...
  if (s?.kind === 'watchdog_timesheets'){
    await runTimesheetsWatchdog({ mode: s.mode || 'weekly', unit: s.unit || 'ALL', recipients: s.recipients || [], hoursPerDay: Number(s.hoursPerDay||8) })
    return
  }
